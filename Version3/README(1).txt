README: extract_csv_final.sh

Overview:
extract_csv_final.sh is a multi-pattern IP report generator that processes CSV or TSV files and creates structured reports. It supports three patterns:
...
./extract_csv_final.sh -p1 input.csv

# Pattern 1 with display name and tab delimiter
./extract_csv_final.sh -p1 input.csv --use-display -dt

# Pattern 2 basic usage
./extract_csv_final.sh -p2 input.csv

# Pattern 2 with comma output delimiter
./extract_csv_final.sh -p2 input.csv -dc

# Pattern 3 basic usage
./extract_csv_final.sh -p3 input.csv

# Pattern 3 with timestamp
./extract_csv_final.sh -p3 input.csv -t

# Show version
./extract_csv_final.sh -v

# Debug mode example
./extract_csv_final.sh -p3 input.csv -d
