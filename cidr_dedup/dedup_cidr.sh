...
1. Basic deduplication (full IP match):
   ./dedup_cidr.sh networks.txt unique_networks.txt
   → Removes duplicates based on full IP address.

2. Ignore specific CIDRs:
   ./dedup_cidr.sh -o "/32,/31" networks.txt unique_networks.txt
   → Removes entries with /32 and /31 CIDRs.

3. Deduplicate by first 3 octets, keep first occurrence:
   ./dedup_cidr.sh -m 3 -k first networks.txt unique_networks.txt
   → Groups by first 3 octets (e.g., 10.1.90.x), keeps first entry.

4. Deduplicate by first 3 octets, keep last occurrence:
   ./dedup_cidr.sh -m 3 -k last networks.txt unique_networks.txt
   → Groups by first 3 octets, keeps last entry.

5. Auto timestamp output filename:
   ./dedup_cidr.sh -t networks.txt
   → Output file becomes network_list-MM-DD_HH-MM-AMPM.txt.

6. Generate summary report with counts:
   ./dedup_cidr.sh -m 3 -r -t networks.txt
   → Creates dedup file and report file with counts per prefix.

7. Generate detailed report with dropped networks:
   ./dedup_cidr.sh -m 3 -rv -t networks.txt
   → Report includes dropped networks under each prefix.

8. Full debug mode with all options:
   ./dedup_cidr.sh -o "/32,/31" -m 3 -k last -rv -t -d networks.txt
   → Shows all operations in stdout, creates dedup and detailed report files.
EOF
}
...
echo "$filtered" | awk -F'[./]' "$prefix_cmd" | sort -u | while read prefix; do
    ...
    if $DEBUG; then
        echo "[DEBUG] Prefix: $prefix -> Kept: $kept | Total: $count"
    fi
done
